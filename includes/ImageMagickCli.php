...
final class ImageMagickCli
{
    ...
    public static function detectCandidates(?array $env = null): array
    {
        ...
        foreach (['magick', 'convert-im7', 'convert-im6', 'convert'] as $name) {
    ...
    {
        $order = ['magick', 'convert-im7', 'convert-im6', 'convert'];
    ...
    private static function probeDefineStrategy(string $bin, array $env): array
    {
        ...
        $chosen = 'none';
        foreach (['heic', 'avif', 'none'] as $ns) {
            if (self::probeConversion($bin, $env, $tests[$ns] ?? [])) {
                $chosen = $ns;
                break;
            }
        }
        ...
        $losslessArgs = $chosen === 'heic'
            ? array_merge($tests[$chosen], [['-define', 'heic:lossless=true']])
            : array_merge($tests[$chosen], [['-define', 'avif:lossless=true']]);
        ...
        $depthArgs = array_merge([['-depth', '10']], $tests[$chosen]);
        $result['supports_depth'] = self::probeConversion($bin, $env, $depthArgs);

        $bitDepthKey = $chosen === 'heic' ? 'heic:bit-depth=10' : 'avif:bit-depth=10';
        $bitDepthArgs = array_merge($tests[$chosen], [['-define', $bitDepthKey]]);
    ...
    private static function probeConversion(string $bin, array $env, array $extraArgsPairs): bool
    {
        ...
        $ok = ($code === 0 && @file_exists($out) && @filesize($out) > 64);
        @unlink($out);
        return $ok;
    }
    ...
    {
        [$code, $output] = self::run($path, ['-version'], $env);
        if ($code !== 0 || $output === '') {
            return '';
        }
        $lines = preg_split("/\r\n|\r|\n/", $output) ?: [];
    ...
    {
        [$code, $output] = self::run($path, ['-list', 'format'], $env);
        if ($code !== 0 || $output === '') {
            return false;
        }
        $lines = preg_split("/\r\n|\r|\n/", $output) ?: [];
