...
class Thumbhash
{
	...
	public static function RGBAToHash($w, $h, $rgba)
	{
		...
		list($a_dc, $a_ac, $a_scale) = $hasAlpha ? static::encodeChannel($a, 5, 5, $w, $h) : array(0, 0, 0);
		...
		$hash = array($header24 & 255, ($header24 >> 8) & 255, $header24 >> 16, $header16 & 255, $header16 >> 8);
		...
		$acList = $hasAlpha ? array($l_ac, $p_ac, $q_ac, $a_ac) : array($l_ac, $p_ac, $q_ac);
	...
	public static function rgbaToDataURL(int $w, int $h, $rgba)
	{
		...
		$ranges = array(array(12, 29), array(37, 41 + $idat));
